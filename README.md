...
3. **로그 실시간 처리**
   - `spark_job/main.py`가 Structured Streaming으로 `logs.*` 패턴 구독, `fact_log.parse_fact_log()`로 스키마 정규화 수행.
   - Spark 스트림의 `/data/log-etlm/spark_checkpoints` 체크포인트 활용, 장애 복구 시점 유지.
4. **로그 저장**
   - `spark_job/warehouse/writer/fact_writer.py` → `spark_job/warehouse/sink.py`의 `write_to_clickhouse()`가 ClickHouse `analytics.fact_log` 테이블에 JDBC append 수행.
   - 초기 스키마는 `spark_job/warehouse/create_tables.sql`로 자동 생성, `/data/log-etlm/clickhouse` 볼륨 영속화.
5. **로그 시각화 및 모니터링**

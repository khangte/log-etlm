...
### 실험 7 — 트리거 4s (G1GC + ClickHouse OOM 수정 동시 적용)
...
| PRE_COALESCE_PARTITIONS | 2 |
| 트리거 인터벌 | 4초 |
| 기타 변경 | G1GC 유지, ClickHouse `background_pool_size=4` + `max_server_memory_usage=0` 유지 |

**결과** (batch 3~146, 워밍업 batch 1-2 제외 기준)
...
**관찰**
- 실험 5(5s + G1GC, ClickHouse OOM 미수정)에서 cascading이 재발했던 원인은 GC가 아니라 **ClickHouse write 스파이크**(OOM 미수정 상태)였음을 사후 확인
- G1GC + ClickHouse OOM 수정을 동시에 적용하면 write 분산이 안정화되어 4s 트리거도 0% 초과율 달성
- 실험 4(6s + G1GC, 최대 4.62s)보다 최대 총시간이 오히려 낮음 — 당시 ClickHouse write 변동이 스파이크 원인이었음을 시사
- **결론: 4s 트리거는 G1GC와 ClickHouse OOM 수정 두 조건이 모두 충족될 때 안정적으로 동작**

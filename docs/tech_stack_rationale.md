...
- **foreachBatch dedup**: `dropDuplicates(["event_id"])`로 배치 단위 중복 제거. 상태 저장소 없이 매 배치 내 중복만 처리해 state I/O 오버헤드 제거. 상태 기반 워터마크 dedup은 `SPARK_FACT_DEDUP_WATERMARK` 설정 시 선택 가능
...
- 대시보드별 새로고침 주기 분리(ops 1분 / realtime 30초 / dim 비활성)로 ClickHouse 읽기 부하를 의도적으로 제한. INSERT 지연 스파이크 사고 이후 주기 확대(ops 30초→1분, realtime 10초→30초)

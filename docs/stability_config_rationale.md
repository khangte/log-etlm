...
`TARGET_INTERVAL_SEC=0.50`, `PUBLISHER_WORKERS=3`, `WORKER_BATCH_SIZE=600` 기준 초당 최대 처리량은  
`3 × 600 / 0.5 = 3,600 events/s`이다.  
...
`event_id`는 UUID가 아닌 콘텐츠 해시(`blake2b-128`)다. seed에는 `request_id`(per-request), `timestamp_ms`(ms 단위), `service`, `event_name` 등 다수 필드가 포함된다.  
